...
public class MovieAPI {
    ...
    private String buildUrl(UUID id) {
        return new MovieAPIBuilder(URL).setID(id).build();
    }

    private static String buildUrl(String query, Genre genre, String releaseYear, String ratingFrom) {
        return new MovieAPIBuilder(URL).setQuery(query).setGenre(genre)
                .setReleaseYear(releaseYear).setRatingForm(ratingFrom).build();
    }
